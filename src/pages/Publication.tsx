...
export default function Publication() {
  ...
  useEffect(() => {
    // Scroll to top when publicationId changes
    window.scrollTo(0, 0);
    if (publicationId && (!publication || publication.id !== publicationId)) {
      const foundPublication = getPublicationContent(publicationId);
      setPublication(foundPublication);
      setActiveSectionId(foundPublication?.pageContent?.sections?.[0]?.id ?? "");
    } else {
      setPublication(null);
    }
  }, [publicationId]);
